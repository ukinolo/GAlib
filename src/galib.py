...
from src.infrastructure import Strategy, Method, Parameter
...
class Selection():
    ...
    _roulette_wheel_method = Method('roulette wheel',
    ...
    def _scaled_roulette_wheel_selection(self, population: np.array, fitness: Callable, number_of_individuals_to_select: int):
        ...
        return np.array(random.choices(population, weights, k=number_of_individuals_to_select))
    _scaled_roulette_wheel_method = Method('scaled roulette wheel',
    ...
    def _ranking_selection(self, population: np.array, fitness: Fitness, number_of_individuals_to_select: int):
        ...
        return np.array(random.choices(population, weights, k=number_of_individuals_to_select))
    _ranking_method = Method('ranking',
                             _ranking_selection,
                             parameters=[
                                 Parameter('max_rank_multiplier', (float, ), 20.0)
                             ])
    ...
    def _tournament_selection(self, population: np.array, fitness: Fitness, number_of_individuals_to_select: int):
        ...
        return mating_pool
    _tournament_method = Method('tournament',
                                _tournament_selection,
                                parameters=[
                                    Parameter('tournament_size', (int, ), 3)
                                ])
    ...
    _random_method = Method('random',
                            _random_selection,
                            parameters=[])

    _info = Strategy('Selection')
...
class Crossover():
    def _symmetrical_crossover(self, mating_pool: np.array, fitness: Fitness):
        ...
        return offspring
    _symmetrical_method = Method('symmetrical',
    ...
    def _bland_crossover(self, mating_pool: np.array, fitness: Fitness):
        ...
        return offspring
    _bland_method = Method('bland',
                           _bland_crossover,
                           parameters=[
                               Parameter('alpha', (float, ), 0.5)
                           ])
    ...
    def _sbx(self, mating_pool: np.array, fitness: Fitness):
        ...
        return offspring
    _sbx_method = Method('sbx',
                         _sbx,
                         parameters=[
                             Parameter('distribution_index', (float, ), 1.5),
                             Parameter('probability_of_crossover', (float, ), 0.5)
                         ])
    
    _info = Strategy('Crossover')
...
class Replacement():
    ...
    _elitism_method = Method('elitism',
                             _elitism_replacement,
                             parameters=[
                                 Parameter('elitism_width', (float, ), 0.05)
                             ])
...
class Mutation():
    ...
    _random_method = Method('random',
                            _random_mutation,
                            parameters=[
                                Parameter('mutation_strength', (float, ), 0.2)
                            ])
    ...
    _nonuniform_method = Method('nonuniform',
                                _nonuniform_mutation,
                                parameters=[
                                    Parameter('mutation_strength', (float, ), 0.2),
                                    Parameter('b', (float, ), 1.0)
                                ])
    ...
    _normally_distributed_method = Method('normal',
                                          _normally_distributed_mutation,
                                          parameters=[
                                              Parameter('standard_deviation', (float, ), 1.0)
                                          ])

    _info = Strategy('Mutation')
    _info.add_method(_random_method)
